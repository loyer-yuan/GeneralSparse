...
warp_bit_map_operator::warp_bit_map_operator(shared_ptr<code_generator> code_generator_ptr, unsigned int coarsen_factor, bool relative_nz, bool relative_row, shared_ptr<operator_context> operator_history)
    : basic_operator("warp_bit_map_operator", code_generator_ptr->get_metadata_set(), IMPLEMENTING_OP, code_generator_ptr->get_sub_matrix_id())
{
...
bool warp_bit_map_operator::is_valid_according_to_operator(shared_ptr<operator_context> operator_history)
{
    ...
    if (thread_flag == true && warp_flag == false)
    {
        return true;
    }
...
bool warp_bit_map_operator::is_valid_according_to_metadata()
{
...
void warp_bit_map_operator::run(bool check)
{
    ...
    shared_ptr<parent_bit_map_of_thread> parent_bit_map_of_thread_ptr(new parent_bit_map_of_thread(this->meta_data_set_ptr,WARP_META, this->target_matrix_id));
    ...
    shared_ptr<warp_bit_map_reduce_token> warp_bit_map_reduce(new warp_bit_map_reduce_token(this->meta_data_set_ptr, this->coarsen_factor, this->code_generator_ptr));

    this->code_generator_ptr->set_reduction_token(WARP_META, warp_bit_map_reduce);
    this->code_generator_ptr->open_spec_level_of_paral(WARP_META);

    this->is_run = true;
}

vector<shared_ptr<transform_step_record_item>> warp_bit_map_operator::get_data_transform_sequence()
{
    assert(this->target_matrix_id >= 0);
...
string warp_bit_map_operator::convert_to_string()
{
    assert(this->target_matrix_id >= 0);

    string return_str = "warp_bit_map_operator::{name:\"" + this->name + "\",stage:" + convert_operator_stage_type_to_string(this->stage) + ",target_matrix_id:" + to_string(target_matrix_id) + "}";
