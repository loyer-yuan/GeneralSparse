...
merge_path_tblock_operator::merge_path_tblock_operator(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, int work_size)
:basic_operator("merge_path_tblock_operator", meta_data_set_ptr, DISTRIBUTING_OP, target_matrix_id)
{
...
merge_path_tblock_operator::merge_path_tblock_operator(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, int work_size, shared_ptr<operator_context> operator_history)
:basic_operator("merge_path_tblock_operator", meta_data_set_ptr, DISTRIBUTING_OP, target_matrix_id)
{
    assert(target_matrix_id >= 0);
    assert(work_size > 0);
...
merge_path_tblock_operator::merge_path_tblock_operator(shared_ptr<code_generator> code_generator_ptr, int work_size, shared_ptr<operator_context> operator_history)
:basic_operator("merge_path_tblock_operator", code_generator_ptr->get_metadata_set(), DISTRIBUTING_OP, code_generator_ptr->get_sub_matrix_id())
{
    new(this)merge_path_tblock_operator(code_generator_ptr->get_metadata_set(), code_generator_ptr->get_sub_matrix_id(), work_size, operator_history);
    this->code_generator_ptr = code_generator_ptr;
}



bool merge_path_tblock_operator::is_valid_according_to_operator(shared_ptr<operator_context> operator_history)
{
...
bool merge_path_tblock_operator::is_valid_according_to_metadata()
{
...
// 执行
void merge_path_tblock_operator::run(bool check)
{
    ...
    // 然后执行分块，首先产生行索引分块
    shared_ptr<get_begin_rows_of_level_after_merge_path> get_begin_rows_tranform_ptr(new get_begin_rows_of_level_after_merge_path(this->meta_data_set_ptr, this->target_matrix_id, TBLOCK_META, this->work_size));
    ...
    // 值索引的偏移
    shared_ptr<get_begin_nzs_of_level_after_merge_path> get_begin_nzs_tranform_ptr(new get_begin_nzs_of_level_after_merge_path(this->meta_data_set_ptr, this->target_matrix_id, TBLOCK_META, this->work_size));
...
vector<shared_ptr<transform_step_record_item>> merge_path_tblock_operator::get_data_transform_sequence()
{
    assert(this->target_matrix_id >= 0);
...
string merge_path_tblock_operator::convert_to_string()
{
    assert(this->work_size > 0);
    assert(this->target_matrix_id >= 0);

    string return_str = "merge_path_tblock_operator::{name:\"" + this->name + "\",stage:" + convert_operator_stage_type_to_string(this->stage) + ",target_matrix_id:" + to_string(target_matrix_id) + ",work_size:" + to_string(this->work_size) + "}";
