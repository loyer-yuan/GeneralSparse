...
sort_operator::sort_operator(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id)
    : basic_operator("sort_operator", meta_data_set_ptr, CONVERTING_OP, target_matrix_id)
{
    assert(target_matrix_id >= 0);
}

sort_operator::sort_operator(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, shared_ptr<operator_context> operator_history)
    : basic_operator("sort_operator", meta_data_set_ptr, CONVERTING_OP, target_matrix_id)
{
    assert(target_matrix_id >= 0);
}

sort_operator::sort_operator(shared_ptr<code_generator> code_generator_ptr, shared_ptr<operator_context> operator_history)
    : basic_operator("sort_operator", code_generator_ptr->get_metadata_set(), CONVERTING_OP, code_generator_ptr->get_sub_matrix_id())
{
}

bool sort_operator::is_valid_according_to_operator(shared_ptr<operator_context> operator_history)
{
...
bool sort_operator::is_valid_according_to_metadata()
{
...
// 执行具体的排序操作
void sort_operator::run(bool check)
{
    if (check)
    {
        assert(this->meta_data_set_ptr != NULL);
    ...
    // 首先获得排序前后的行索引映，创建一个data_transform
    shared_ptr<get_row_order_by_length> get_row_order_by_length_transform(new get_row_order_by_length(this->meta_data_set_ptr, this->target_matrix_id));
    // 然后执行对应的data_transform
    get_row_order_by_length_transform->run(check);
    this->set_transform_seq(get_record_item_of_a_transform_step(get_row_order_by_length_transform));
...
vector<shared_ptr<transform_step_record_item>> sort_operator::get_data_transform_sequence()
{
    assert(this->target_matrix_id >= 0);
...
string sort_operator::convert_to_string()
{
    assert(this->target_matrix_id >= 0);

    string return_str = "sort_operator::{name:\"" + this->name + "\",stage:" + convert_operator_stage_type_to_string(this->stage) + ",target_matrix_id:" + to_string(target_matrix_id) + "}";
