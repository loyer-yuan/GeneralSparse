...
balanced_interval_row_direction_thread_blocking_operator::balanced_interval_row_direction_thread_blocking_operator(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, int nnz_per_interval, bool row_index_is_relative_to_parent, bool nz_index_is_relative_to_parent)
    : basic_operator("balanced_interval_row_direction_thread_blocking_operator", meta_data_set_ptr, DISTRIBUTING_OP, target_matrix_id)
{
...
balanced_interval_row_direction_thread_blocking_operator::balanced_interval_row_direction_thread_blocking_operator(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, int nnz_per_interval, bool row_index_is_relative, bool nz_index_is_relative,  shared_ptr<operator_context> operator_history)
    : basic_operator("balanced_interval_row_direction_thread_blocking_operator", meta_data_set_ptr, DISTRIBUTING_OP, target_matrix_id)
{
    assert(target_matrix_id >= 0);
    assert(nnz_per_interval > 0);
...
balanced_interval_row_direction_thread_blocking_operator::balanced_interval_row_direction_thread_blocking_operator(shared_ptr<code_generator> code_generator_ptr, int nnz_per_interval, bool row_index_is_relative, bool nz_index_is_relative,  shared_ptr<operator_context> operator_history)
    : basic_operator("balanced_interval_row_direction_thread_blocking_operator", code_generator_ptr->get_metadata_set(), DISTRIBUTING_OP, code_generator_ptr->get_sub_matrix_id())
{
    new(this) balanced_interval_row_direction_thread_blocking_operator(code_generator_ptr->get_metadata_set(),code_generator_ptr->get_sub_matrix_id(), nnz_per_interval, row_index_is_relative, nz_index_is_relative, operator_history);
    this->code_generator_ptr = code_generator_ptr;

}


bool balanced_interval_row_direction_thread_blocking_operator::is_valid_according_to_operator(shared_ptr<operator_context> operator_history)
{
    ...
    if (former_operator_implementing.size() == 0)
    {
        ...
        if (thread_flag == false && col_direction_flag == false && interlance_flag == false && nnz_direction_flag == false)
        {
            return true;
        }
    }
...
bool balanced_interval_row_direction_thread_blocking_operator::is_valid_according_to_metadata()
{
    ...
    if (this->row_index_is_relative_to_parent == true || this->nz_index_is_relative_to_parent == true)
    {
...
// 执行
void balanced_interval_row_direction_thread_blocking_operator::run(bool check)
{
    if (check)
    {
    ...
    if (BMW_row_begin_existing == true)
    {
        ...
        // 获得thread级别的首行绝对索引
        shared_ptr<get_begin_rows_of_BMT_after_nnz_blocking_in_row_direction_in_BMW> get_absolute_BMT_start_row_ptr(new get_begin_rows_of_BMT_after_nnz_blocking_in_row_direction_in_BMW(this->meta_data_set_ptr, this->target_matrix_id, this->nnz_per_interval));
        ...
        {
            // 获得相对行索引
            shared_ptr<get_begin_rows_of_BMT_after_nnz_blocking_in_row_direction_relative_to_BMW> get_relative_BMT_start_row_ptr(new get_begin_rows_of_BMT_after_nnz_blocking_in_row_direction_relative_to_BMW(this->meta_data_set_ptr, this->target_matrix_id, this->nnz_per_interval));
        ...
        // thread级别的绝对非零元索引
        shared_ptr<get_begin_nzs_of_BMT_after_nnz_blocking_in_row_direction_in_BMW> get_absolute_BMT_start_nz_ptr(new get_begin_nzs_of_BMT_after_nnz_blocking_in_row_direction_in_BMW(this->meta_data_set_ptr, this->target_matrix_id, this->nnz_per_interval));
        ...
        {
            // 获得相对非零元索引
            shared_ptr<get_begin_nzs_of_BMT_after_nnz_blocking_in_row_direction_relative_to_BMW> get_relative_BMT_start_nz_ptr(new get_begin_nzs_of_BMT_after_nnz_blocking_in_row_direction_relative_to_BMW(this->meta_data_set_ptr, this->target_matrix_id, this->nnz_per_interval));
    ...
    else if (BMTB_row_begin_existing == true)
    {
        ...
        // 获得thread级别的首行绝对索引
        shared_ptr<get_begin_rows_of_BMT_after_nnz_blocking_in_row_direction_in_BMTB> get_absolute_BMT_start_row_ptr(new get_begin_rows_of_BMT_after_nnz_blocking_in_row_direction_in_BMTB(this->meta_data_set_ptr, this->target_matrix_id, this->nnz_per_interval));
        get_absolute_BMT_start_row_ptr->run(check);
        this->set_transform_seq(get_record_item_of_a_transform_step(get_absolute_BMT_start_row_ptr));
        ...
        {
            // 获得相对行索引
            shared_ptr<get_begin_rows_of_BMT_after_nnz_blocking_in_row_direction_relative_to_BMTB> get_relative_BMT_start_row_ptr(new get_begin_rows_of_BMT_after_nnz_blocking_in_row_direction_relative_to_BMTB(this->meta_data_set_ptr, this->target_matrix_id, this->nnz_per_interval));
            get_relative_BMT_start_row_ptr->run(check);
            this->set_transform_seq(get_record_item_of_a_transform_step(get_relative_BMT_start_row_ptr));
        }

        // thread级别的绝对非零元索引
        shared_ptr<get_begin_nzs_of_BMT_after_nnz_blocking_in_row_direction_in_BMTB> get_absolute_BMT_start_nz_ptr(new get_begin_nzs_of_BMT_after_nnz_blocking_in_row_direction_in_BMTB(this->meta_data_set_ptr, this->target_matrix_id, this->nnz_per_interval));
        get_absolute_BMT_start_nz_ptr->run(check);
        this->set_transform_seq(get_record_item_of_a_transform_step(get_absolute_BMT_start_nz_ptr));
        ...
        {
            // 获得相对非零元索引
            shared_ptr<get_begin_nzs_of_BMT_after_nnz_blocking_in_row_direction_relative_to_BMTB> get_relative_BMT_start_nz_ptr(new get_begin_nzs_of_BMT_after_nnz_blocking_in_row_direction_relative_to_BMTB(this->meta_data_set_ptr, this->target_matrix_id, this->nnz_per_interval));
            get_relative_BMT_start_nz_ptr->run(check);
            this->set_transform_seq(get_record_item_of_a_transform_step(get_relative_BMT_start_nz_ptr));
        }
    ...
    else
    {
        ...
        shared_ptr<get_begin_rows_of_BMT_after_nnz_blocking_in_row_direction> get_absolute_BMT_start_row_ptr(new get_begin_rows_of_BMT_after_nnz_blocking_in_row_direction(this->meta_data_set_ptr, this->target_matrix_id, this->nnz_per_interval));
        get_absolute_BMT_start_row_ptr->run(check);
        this->set_transform_seq(get_record_item_of_a_transform_step(get_absolute_BMT_start_row_ptr));

        shared_ptr<get_begin_nzs_of_BMT_after_nnz_blocking_in_row_direction> get_absolute_BMT_start_nz_ptr(new get_begin_nzs_of_BMT_after_nnz_blocking_in_row_direction(this->meta_data_set_ptr, this->target_matrix_id, this->nnz_per_interval));
        get_absolute_BMT_start_nz_ptr->run(check);
        this->set_transform_seq(get_record_item_of_a_transform_step(get_absolute_BMT_start_nz_ptr));
    }
    if (check)
...
vector<shared_ptr<transform_step_record_item>> balanced_interval_row_direction_thread_blocking_operator::get_data_transform_sequence()
{
    assert(this->target_matrix_id >= 0);
...
string balanced_interval_row_direction_thread_blocking_operator::convert_to_string()
{
    assert(this->nnz_per_interval > 0);
    assert(this->target_matrix_id >= 0);

    string return_str = "balanced_interval_row_direction_thread_blocking_operator::{name:\"" + this->name + "\",stage:" + convert_operator_stage_type_to_string(this->stage) + ",target_matrix_id:" + to_string(target_matrix_id) + ",nnz_per_interval:" + to_string(this->nnz_per_interval)  + "}";
