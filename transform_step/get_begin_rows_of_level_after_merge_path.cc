...
get_begin_rows_of_level_after_merge_path::get_begin_rows_of_level_after_merge_path(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, POS_TYPE pos, int work_size)
    : basic_data_transform_step("get_begin_rows_of_level_after_merge_path", meta_data_set_ptr)
{
...
void get_begin_rows_of_level_after_merge_path::run(bool check)
{
    if (check)
    {
    ...
    vector<unsigned long> path_row_vec;
    ...
    for(unsigned long i = 0; i < row_num; i++)
    {
        ...
        if (nnz_of_each_row[i] != 0)
        {
            ...
            path_row_vec.push_back(i);
        }
    }


    vector<unsigned long> level_begin_row_vec;
    ...
    for(unsigned long i = 0; i < count; i += this->work_size)
    {
        for(unsigned long j = 0; j < total_path.size(); j++)
        {
            if (total_path[j] > i)
            {
                level_begin_row_vec.push_back(path_row_vec[j]);
                break;
            }
        }
    }
    // 将数据放到metadata set中
    shared_ptr<universal_array> level_first_row_ptr(new universal_array(&(level_begin_row_vec[0]), level_begin_row_vec.size(), UNSIGNED_LONG));
    shared_ptr<meta_data_item> level_first_row_item(new meta_data_item(level_first_row_ptr, pos, "first_row_indices_without_ending", this->target_matrix_id));
    this->meta_data_set_ptr->add_element(level_first_row_item);

    // 执行记录
    shared_ptr<data_item_record> level_first_row_record(new data_item_record(pos, "first_row_indices_without_ending", this->target_matrix_id));
    this->dest_data_item_ptr_vec.push_back(level_first_row_record);

    this->is_run = true;
}

vector<shared_ptr<data_item_record>> get_begin_rows_of_level_after_merge_path::get_source_data_item_ptr_in_data_transform_step()
{
...
vector<shared_ptr<data_item_record>> get_begin_rows_of_level_after_merge_path::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    assert(this->meta_data_set_ptr != NULL);
    assert(this->meta_data_set_ptr->check());
    assert(this->target_matrix_id >= 0);
...
string get_begin_rows_of_level_after_merge_path::convert_to_string()
{
    assert(this->target_matrix_id >= 0);

    string return_str = "get_begin_rows_of_level_after_merge_path::{name:\"" + name + "\",target_matrix_id:" + to_string(target_matrix_id) + ",work_size:" + to_string(this->work_size) + "}";
