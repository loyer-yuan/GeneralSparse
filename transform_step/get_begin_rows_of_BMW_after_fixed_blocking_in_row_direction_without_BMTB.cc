...
get_begin_rows_of_BMW_after_fixed_blocking_in_row_direction_without_BMTB::get_begin_rows_of_BMW_after_fixed_blocking_in_row_direction_without_BMTB(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, int fixed_row_block_size)
    : basic_data_transform_step("get_begin_rows_of_BMW_after_fixed_blocking_in_row_direction_without_BMTB", meta_data_set_ptr)
{
...
void get_begin_rows_of_BMW_after_fixed_blocking_in_row_direction_without_BMTB::run(bool check)
{
    if (check)
    {
        assert(target_matrix_id >= 0);
    ...
    // 产生一个新的metadata item
    shared_ptr<universal_array> BMW_begin_row_index_ptr(new universal_array(&(row_begin_index_vec[0]), row_begin_index_vec.size(), UNSIGNED_LONG));
    shared_ptr<meta_data_item> item_ptr(new meta_data_item(BMW_begin_row_index_ptr, WARP_META, "first_row_indices", this->target_matrix_id));
...
vector<shared_ptr<data_item_record>> get_begin_rows_of_BMW_after_fixed_blocking_in_row_direction_without_BMTB::get_source_data_item_ptr_in_data_transform_step()
{
...
vector<shared_ptr<data_item_record>> get_begin_rows_of_BMW_after_fixed_blocking_in_row_direction_without_BMTB::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    assert(this->meta_data_set_ptr != NULL);
    assert(this->meta_data_set_ptr->check());
    assert(this->target_matrix_id >= 0);
...
string get_begin_rows_of_BMW_after_fixed_blocking_in_row_direction_without_BMTB::convert_to_string()
{
    assert(this->target_matrix_id >= 0);
    assert(this->fixed_row_block_size >= 1);

    string return_str = "get_begin_rows_of_BMW_after_fixed_blocking_in_row_direction_without_BMTB::{name:\"" + name + "\",target_matrix_id:" + to_string(target_matrix_id) + ",fixed_row_block_size:" + to_string(this->fixed_row_block_size) + "}";
