...
get_begin_rows_of_BMW_after_fixed_blocking_in_col_direction::get_begin_rows_of_BMW_after_fixed_blocking_in_col_direction(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, int col_size)
    : basic_data_transform_step("get_begin_rows_of_BMW_after_fixed_blocking_in_col_direction", meta_data_set_ptr)
{
...
void get_begin_rows_of_BMW_after_fixed_blocking_in_col_direction::run(bool check)
{
    if (check)
    {
        assert(target_matrix_id >= 0);
    ...
    for (unsigned long i = 0; i < nnz_of_each_row.size(); i++)
    {
        ...
        {
            BMW_begin_row_vec.push_back(i);
        }
    }

    // 将数据放到metadata set中
    shared_ptr<universal_array> BMW_first_row_ptr(new universal_array(&(BMW_begin_row_vec[0]), BMW_begin_row_vec.size(), UNSIGNED_LONG));
    shared_ptr<meta_data_item> BMW_first_row_item(new meta_data_item(BMW_first_row_ptr, WARP_META, "first_row_indices_without_ending", this->target_matrix_id));
    this->meta_data_set_ptr->add_element(BMW_first_row_item);

    // 执行记录
    shared_ptr<data_item_record> BMW_first_row_without_end_record(new data_item_record(WARP_META, "first_row_indices_without_ending", this->target_matrix_id));
    this->dest_data_item_ptr_vec.push_back(BMW_first_row_without_end_record);

    this->is_run = true;
}

vector<shared_ptr<data_item_record>> get_begin_rows_of_BMW_after_fixed_blocking_in_col_direction::get_source_data_item_ptr_in_data_transform_step()
{
...
vector<shared_ptr<data_item_record>> get_begin_rows_of_BMW_after_fixed_blocking_in_col_direction::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    assert(this->target_matrix_id >= 0);
    assert(this->fixed_col_block_size > 0);
    assert(this->is_run == true);
...
string get_begin_rows_of_BMW_after_fixed_blocking_in_col_direction::convert_to_string()
{
    assert(this->target_matrix_id >= 0);
    assert(this->fixed_col_block_size > 0);

    string return_str = "get_begin_rows_of_BMW_after_fixed_blocking_in_col_direction::{name:\"" + name + "\",target_matrix_id:" + to_string(target_matrix_id) + ",col_size:" + to_string(this->fixed_col_block_size) + "}";
