...
get_begin_rows_of_BMW_after_fixed_blocking_in_row_direction_in_BMTB::get_begin_rows_of_BMW_after_fixed_blocking_in_row_direction_in_BMTB(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, int fixed_row_block_size)
    : basic_data_transform_step("get_begin_rows_of_BMW_after_fixed_blocking_in_row_direction_in_BMTB", meta_data_set_ptr)
{
...
void get_begin_rows_of_BMW_after_fixed_blocking_in_row_direction_in_BMTB::run(bool check)
{
    if (check)
    {
        assert(target_matrix_id >= 0);
    ...
    // 遍历所有BMTB，在nz_begin_index_of_each_BMW中插入BMW和BMTB的行边界
    for (unsigned long i = 0; i < BMTB_first_row_indices_ptr->get_len() - 1; i++)
    {
        ...
        {
            // 相对和直接的方式不一样
            // if (this->is_relative_to_BMTB == true)
            // {
            //     nz_begin_index_of_each_BMW_relative_to_BMTB.push_back(BMW_first_row_index - BMTB_first_row_index);
            // }

            nz_begin_index_of_each_BMW.push_back(BMW_first_row_index);
        }
    }

    // 绝对索引最后一位不可能是总的行数量，加入这一边界
    if (check)
    {
        assert(nz_begin_index_of_each_BMW[nz_begin_index_of_each_BMW.size() - 1] != BMTB_first_row_indices_ptr->read_integer_from_arr(BMTB_first_row_indices_ptr->get_len() - 1));
    }
    nz_begin_index_of_each_BMW.push_back(BMTB_first_row_indices_ptr->read_integer_from_arr(BMTB_first_row_indices_ptr->get_len() - 1));

    // 添加新的metadata
    // 创造通用数组
    shared_ptr<universal_array> BMW_first_row_index_ptr(new universal_array(&(nz_begin_index_of_each_BMW[0]), nz_begin_index_of_each_BMW.size(), UNSIGNED_LONG));
    // BMW行偏移的全局索引
    shared_ptr<meta_data_item> BMW_first_row_index_item_ptr(new meta_data_item(BMW_first_row_index_ptr, WARP_META, "first_row_indices", this->target_matrix_id));
    this->meta_data_set_ptr->add_element(BMW_first_row_index_item_ptr);

    // if (this->is_relative_to_BMTB == true)
    // {
    //     assert(nz_begin_index_of_each_BMW_relative_to_BMTB.size() > 0);
    ...
    //     shared_ptr<meta_data_item> BMW_first_row_index_relative_to_BMTB_item_ptr(new meta_data_item(BMW_first_row_index_relative_to_BMTB_ptr, WARP_META, "first_row_indices_relative_to_BMTB", this->target_matrix_id));
    //     this->meta_data_set_ptr->add_element(BMW_first_row_index_relative_to_BMTB_item_ptr);
    // }
...
// 输入是所有元素的行索引，BMTB的行边界，子块的行边界。输出必然有BMW的绝对行边界，根据情况判断是不是有BMW相对行边界
vector<shared_ptr<data_item_record>> get_begin_rows_of_BMW_after_fixed_blocking_in_row_direction_in_BMTB::get_source_data_item_ptr_in_data_transform_step()
{
...
// 输入是所有元素的行索引，BMTB的行边界，子块的行边界。输出必然有BMW的绝对行边界，根据情况判断是不是有BMW相对行边界
vector<shared_ptr<data_item_record>> get_begin_rows_of_BMW_after_fixed_blocking_in_row_direction_in_BMTB::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    ...
    // 输出每一BMW的首行索引
    shared_ptr<data_item_record> BMW_begin_row_index_record(new data_item_record(WARP_META, "first_row_indices", this->target_matrix_id));
    record_vec.push_back(BMW_begin_row_index_record);
...
string get_begin_rows_of_BMW_after_fixed_blocking_in_row_direction_in_BMTB::convert_to_string()
{
    assert(this->target_matrix_id >= 0);
    assert(this->fixed_row_block_size >= 1);

    string return_str = "get_begin_rows_of_BMW_after_fixed_blocking_in_row_direction_in_BMTB::{name:\"" + name + "\",target_matrix_id:" + to_string(target_matrix_id) + ",fixed_row_block_size:" + to_string(this->fixed_row_block_size) + "}";
