...
modify_col_indices_by_empty_pad_in_submatrix::modify_col_indices_by_empty_pad_in_submatrix(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id)
    : basic_data_transform_step("modify_col_indices_by_empty_pad_in_submatrix", meta_data_set_ptr)
{
...
void modify_col_indices_by_empty_pad_in_submatrix::run(bool check)
{
    if (check)
    {
        // 保证metadata set
    ...
    for (unsigned long i = 0; i < row_indices_ptr->get_len(); i++)
    {
        ...
        unsigned long nz_col_id = col_indices_ptr->read_integer_from_arr(i);
        if (next_nz_row_id == nz_row_id)
        {
            new_col_indices_vec.push_back(nz_col_id);
            continue;
        }
        ...
        else
        {
            ...
            if (check)
            {
                assert(empty_id < next_nz_row_id);
            }
...
vector<shared_ptr<data_item_record>> modify_col_indices_by_empty_pad_in_submatrix::get_source_data_item_ptr_in_data_transform_step()
{
    // 执行过
...
vector<shared_ptr<data_item_record>> modify_col_indices_by_empty_pad_in_submatrix::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    // 执行过
    assert(this->is_run == true);
    // 内容符合要求
...
string modify_col_indices_by_empty_pad_in_submatrix::convert_to_string()
{
    // 内容符合要求
    assert(this->target_matrix_id >= 0);
    string return_str = "modify_col_indices_by_empty_pad_in_submatrix::{name:\"" + name + "\",target_matrix_id:" + to_string(target_matrix_id) + "}";
    return return_str;
}