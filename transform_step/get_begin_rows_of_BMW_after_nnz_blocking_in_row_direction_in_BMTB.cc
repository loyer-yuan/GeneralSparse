...
get_begin_rows_of_BMW_after_nnz_blocking_in_row_direction_in_BMTB::get_begin_rows_of_BMW_after_nnz_blocking_in_row_direction_in_BMTB(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, int nnz_per_interval)
    : basic_data_transform_step("get_begin_rows_of_BMW_after_nnz_blocking_in_row_direction_in_BMTB", meta_data_set_ptr)
{
...
void get_begin_rows_of_BMW_after_nnz_blocking_in_row_direction_in_BMTB::run(bool check)
{
    if (check)
    {
    ...
    vector<unsigned long> BMW_begin_row_vec = get_begin_rows_of_child_after_balance_blocking_in_row_direction_in_parent(BMTB_first_row_indices_ptr, nnz_of_each_row, this->nnz_per_interval, row_num);
    ...
    shared_ptr<meta_data_item> BMW_first_row_without_end_item(new meta_data_item(BMW_first_row_without_end_ptr, WARP_META, "first_row_indices", this->target_matrix_id));
    this->meta_data_set_ptr->add_element(BMW_first_row_without_end_item);

    // 执行记录
    shared_ptr<data_item_record> BMW_first_row_without_end_record(new data_item_record(WARP_META, "first_row_indices", this->target_matrix_id));
    this->dest_data_item_ptr_vec.push_back(BMW_first_row_without_end_record);

    this->is_run = true;
}

vector<shared_ptr<data_item_record>> get_begin_rows_of_BMW_after_nnz_blocking_in_row_direction_in_BMTB::get_source_data_item_ptr_in_data_transform_step()
{
...
vector<shared_ptr<data_item_record>> get_begin_rows_of_BMW_after_nnz_blocking_in_row_direction_in_BMTB::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    assert(this->meta_data_set_ptr != NULL);
    assert(this->meta_data_set_ptr->check());
    assert(this->target_matrix_id >= 0);
...
string get_begin_rows_of_BMW_after_nnz_blocking_in_row_direction_in_BMTB::convert_to_string()
{
    assert(this->target_matrix_id >= 0);
    assert(this->nnz_per_interval >= 1);

    string return_str = "get_begin_rows_of_BMW_after_nnz_blocking_in_row_direction_in_BMTB::{name:\"" + name + "\",target_matrix_id:" + to_string(target_matrix_id) + ",nnz_per_interval:" + to_string(this->nnz_per_interval) + "}";
