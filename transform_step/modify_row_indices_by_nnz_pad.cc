...
modify_row_indices_by_nnz_pad::modify_row_indices_by_nnz_pad(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, int nnz_target)
    : basic_data_transform_step("modify_row_indices_by_nnz_pad", meta_data_set_ptr)
{
...
void modify_row_indices_by_nnz_pad::run(bool check)
{
    if (check)
    {
        // 保证metadata set
    ...
    unsigned long nz_num = row_indices_ptr->get_len();
    if (nz_num % this->nnz_target != 0)
    {
        ...
        if (check)
        {
            ...
            {
                cout << "modify_row_indices_by_nnz_pad::run(): current padding rate is" << (double)new_nz_number / (double)nz_num << ", higher than" << get_config()["PADDING_RATE_UP_BOUND"].as_integer() << endl;
                assert(false);
            }
        }
        ...
        unsigned long last_id = row_indices_ptr->read_integer_from_arr(row_indices_ptr->get_len() - 1);

        for (unsigned long i = 0; i < added_nz_number; i++)
        {
            new_row_indices_vec.push_back(last_id);
        }
...
vector<shared_ptr<data_item_record>> modify_row_indices_by_nnz_pad::get_source_data_item_ptr_in_data_transform_step()
{
    // 执行过
...
vector<shared_ptr<data_item_record>> modify_row_indices_by_nnz_pad::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    // 执行过
    assert(this->is_run == true);
    // 内容符合要求
...
string modify_row_indices_by_nnz_pad::convert_to_string()
{
    // 内容符合要求
    assert(this->target_matrix_id >= 0);
    string return_str = "modify_row_indices_by_nnz_pad::{name:\"" + name + "\",target_matrix_id:" + to_string(target_matrix_id) + "nnz_target:" + to_string(nnz_target) + "}";
    return return_str;
}