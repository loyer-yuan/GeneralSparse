...
modify_vals_by_row_pad_in_sub_matrix::modify_vals_by_row_pad_in_sub_matrix(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, int multiple)
    : basic_data_transform_step("modify_vals_by_row_pad_in_sub_matrix", meta_data_set_ptr)
{
...
void modify_vals_by_row_pad_in_sub_matrix::run(bool check)
{
    if (check)
    {
        // 保证metadata set
    ...
    if (check)
    {
        ...
        assert(last_row_index < row_num_of_sub_matrix);
          // 当前行索引还没有padding过
        assert(vals_ptr->get_len() == row_indices_ptr->get_len());
    }
    ...
    if (row_num_of_sub_matrix % this->multiple != 0)
    {
        ...
        if (check)
        {
            ...
            {
                cout << "modify_vals_by_row_pad_in_sub_matrix::run(): current padding rate is" << (double)nnz_after_padding / (double)original_nnz << ", higher than" << get_config()["PADDING_RATE_UP_BOUND"].as_integer() << endl;
                assert(false);
            }
        }
        ...
        // 增加输出
        shared_ptr<data_item_record> nz_vals_record1(new data_item_record(GLOBAL_META, "nz_vals", this->target_matrix_id));
        this->dest_data_item_ptr_vec.push_back(nz_vals_record1);
    }

    this->is_run = true;
}

vector<shared_ptr<data_item_record>> modify_vals_by_row_pad_in_sub_matrix::get_source_data_item_ptr_in_data_transform_step()
{
    // 执行过
...
vector<shared_ptr<data_item_record>> modify_vals_by_row_pad_in_sub_matrix::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    // 执行过
    assert(this->is_run == true);
    // 内容符合要求
...
string modify_vals_by_row_pad_in_sub_matrix::convert_to_string()
{
    // 内容符合要求
    assert(this->target_matrix_id >= 0);
    assert(this->multiple >= 2);

    string return_str = "modify_vals_by_row_pad_in_sub_matrix::{name:\"" + name + "\",target_matrix_id:" + to_string(target_matrix_id) + ",multiple:" + to_string(multiple) + "}";
