...
get_begin_BMTs_of_specific_parent_after_blocking::get_begin_BMTs_of_specific_parent_after_blocking(shared_ptr<meta_data_set> meta_data_set_ptr, POS_TYPE parent_pos, int target_matrix_id)
    : basic_data_transform_step("get_begin_BMTs_of_specific_parent_after_blocking", meta_data_set_ptr)
{
...
void get_begin_BMTs_of_specific_parent_after_blocking::run(bool check)
{
    if (check)
    {
        assert(meta_data_set_ptr != NULL);
        ...
        // 利用父块和BMT的非零元偏移量来计算每一个父块中BMT的数量，并且使用这些数量来得到父块中BMT的偏移量
        assert(this->meta_data_set_ptr->is_exist(THREAD_META, "first_nz_indices", this->target_matrix_id));
        assert(this->meta_data_set_ptr->is_exist(parent_pos, "first_nz_indices", this->target_matrix_id));
    }
    ...
    shared_ptr<universal_array> first_nz_indices_of_parent_ptr = this->meta_data_set_ptr->get_element(parent_pos, "first_nz_indices", this->target_matrix_id)->get_metadata_arr();
    ...
    shared_ptr<data_item_record> first_nz_indices_of_parent_record(new data_item_record(parent_pos, "first_nz_indices", this->target_matrix_id));
    this->source_data_item_ptr_vec.push_back(first_nz_indices_of_parent_record);

    // 遍历所有的父块，将BMT偏移量存入
    vector<unsigned long> BMT_offset_of_each_parent;
    BMT_offset_of_each_parent.push_back(0);
    ...
    for (unsigned long parent_blk_id = 0; parent_blk_id < first_nz_indices_of_parent_ptr->get_len() - 1; parent_blk_id++)
    {
        ...
        if (check)
        {
            ...
            {
                cout << "get_begin_BMTs_of_specific_parent_after_blocking::run(): cur_BMT_first_nz_id:" << cur_BMT_first_nz_id << ", cur_parent_blk_first_nz_id:" << cur_parent_blk_first_nz_id << endl;
                assert(false);
            }
        }
        ...
        assert(cur_BMT_first_nz_id == next_parent_blk_first_nz_id);
    ...
    {
        // 检查各个数据的大小
        assert(begin_BMT_index_of_each_parent_vec.size() == first_nz_indices_of_parent_ptr->get_len());
        assert(begin_BMT_index_of_each_parent_vec[begin_BMT_index_of_each_parent_vec.size() - 1] == first_nz_indices_of_BMT_ptr->get_len() - 1);
...
vector<shared_ptr<data_item_record>> get_begin_BMTs_of_specific_parent_after_blocking::get_source_data_item_ptr_in_data_transform_step()
{
    assert(target_matrix_id >= 0);
    assert(parent_pos == TBLOCK_META || parent_pos == WARP_META);
...
vector<shared_ptr<data_item_record>> get_begin_BMTs_of_specific_parent_after_blocking::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    assert(target_matrix_id >= 0);
    assert(parent_pos == TBLOCK_META || parent_pos == WARP_META);
    assert(this->is_run == true);
...
string get_begin_BMTs_of_specific_parent_after_blocking::convert_to_string()
{
    assert(target_matrix_id >= 0);
    assert(parent_pos == TBLOCK_META || parent_pos == WARP_META);
    assert(this->is_run == true);

    string return_str = "get_begin_BMTs_of_specific_parent_after_blocking::{name:\"" + name + "\",target_matrix_id:" + to_string(target_matrix_id) + ",parent_pos:" + convert_pos_type_to_string(this->parent_pos) + "}";
