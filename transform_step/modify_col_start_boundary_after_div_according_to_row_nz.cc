...
modify_col_start_boundary_after_div_according_to_row_nz::modify_col_start_boundary_after_div_according_to_row_nz(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, int nz_gap_size, int max_gap, int expansion_rate)
    : basic_data_transform_step("modify_col_start_boundary_after_div_according_to_row_nz", meta_data_set_ptr)
{
...
void modify_col_start_boundary_after_div_according_to_row_nz::run(bool check)
{
    if (check)
    {
...
vector<shared_ptr<data_item_record>> modify_col_start_boundary_after_div_according_to_row_nz::get_source_data_item_ptr_in_data_transform_step()
{
    assert(this->target_matrix_id >= 0);
...
vector<shared_ptr<data_item_record>> modify_col_start_boundary_after_div_according_to_row_nz::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    assert(this->target_matrix_id >= 0);
    assert(this->is_run == true);
...
string modify_col_start_boundary_after_div_according_to_row_nz::convert_to_string()
{
    assert(this->target_matrix_id >= 0);

    // 打印参数
    string return_str = "modify_col_start_boundary_after_div_according_to_row_nz::{name:\"" + this->name + "\",target_matrix_id:" + to_string(this->target_matrix_id) + ",nz_gap_size:" + to_string(this->nz_gap_size) + "}";
