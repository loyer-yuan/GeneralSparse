...
get_begin_rows_of_BMT_after_fixed_blocking_in_row_direction::get_begin_rows_of_BMT_after_fixed_blocking_in_row_direction(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, int fixed_row_block_size)
    : basic_data_transform_step("get_begin_rows_of_BMT_after_fixed_blocking_in_row_direction", meta_data_set_ptr)
{
...
void get_begin_rows_of_BMT_after_fixed_blocking_in_row_direction::run(bool check)
{
    if (check)
    {
    ...
    for (unsigned long i = 0; i < row_num; i++)
    {
        ...
        if (row_index_in_BMT == this->fixed_row_block_size)
        {
            if (check)
            {
...
vector<shared_ptr<data_item_record>> get_begin_rows_of_BMT_after_fixed_blocking_in_row_direction::get_source_data_item_ptr_in_data_transform_step()
{
...
vector<shared_ptr<data_item_record>> get_begin_rows_of_BMT_after_fixed_blocking_in_row_direction::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    assert(this->meta_data_set_ptr != NULL);
    assert(this->meta_data_set_ptr->check());
    assert(this->target_matrix_id >= 0);
...
string get_begin_rows_of_BMT_after_fixed_blocking_in_row_direction::convert_to_string()
{
    assert(this->target_matrix_id >= 0);
    assert(this->fixed_row_block_size >= 1);

    string return_str = "get_begin_rows_of_BMT_after_fixed_blocking_in_row_direction::{name:\"" + name + "\",target_matrix_id:" + to_string(target_matrix_id) + ",fixed_row_block_size:" + to_string(this->fixed_row_block_size) + "}";
