...
get_begin_nzs_of_BMT_after_fixed_blocking_in_row_direction_relative_to_BMTB::get_begin_nzs_of_BMT_after_fixed_blocking_in_row_direction_relative_to_BMTB(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, int fixed_row_block_size)
    : basic_data_transform_step("get_begin_nzs_of_BMT_after_fixed_blocking_in_row_direction_relative_to_BMTB", meta_data_set_ptr)
{
...
void get_begin_nzs_of_BMT_after_fixed_blocking_in_row_direction_relative_to_BMTB::run(bool check)
{
    if (check)
    {
    ...
    for (unsigned long j = 0; j < BMTB_first_row_indices_ptr->get_len() - 1; j++)
    {
        ...
        // 当前BMTB非零元偏移量
        // unsigned long BMTB_first_nz = BMTB_first_nzs_ptr->read_integer_from_arr(j);
        // unsigned long next_BMTB_first_nz = BMTB_first_nzs_ptr->read_integer_from_arr(j + 1);
        ...
        for (unsigned long i = BMTB_first_row_index; i < next_BMTB_first_row_index; i++)
        {
            ...
            if (row_count == this->fixed_row_block_size && i != (next_BMTB_first_row_index - 1))
            {
    ...
    shared_ptr<meta_data_item> BMT_first_nz_item(new meta_data_item(BMT_first_nz_ptr, THREAD_META, "first_nz_indices_relative_to_BMTB", this->target_matrix_id));
    this->meta_data_set_ptr->add_element(BMT_first_nz_item);

    // 执行记录
    shared_ptr<data_item_record> BMT_first_nz_record(new data_item_record(THREAD_META, "first_nz_indices_relative_to_BMTB", this->target_matrix_id));
    this->dest_data_item_ptr_vec.push_back(BMT_first_nz_record);

    this->is_run = true;
}

vector<shared_ptr<data_item_record>> get_begin_nzs_of_BMT_after_fixed_blocking_in_row_direction_relative_to_BMTB::get_source_data_item_ptr_in_data_transform_step()
{
...
vector<shared_ptr<data_item_record>> get_begin_nzs_of_BMT_after_fixed_blocking_in_row_direction_relative_to_BMTB::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    assert(this->meta_data_set_ptr != NULL);
    assert(this->meta_data_set_ptr->check());
    assert(this->target_matrix_id >= 0);
...
string get_begin_nzs_of_BMT_after_fixed_blocking_in_row_direction_relative_to_BMTB::convert_to_string()
{
    assert(this->target_matrix_id >= 0);
    assert(this->fixed_row_block_size >= 1);

    string return_str = "get_begin_nzs_of_BMT_after_fixed_blocking_in_row_direction_relative_to_BMTB::{name:\"" + name + "\",target_matrix_id:" + to_string(target_matrix_id) + ",fixed_row_block_size:" + to_string(this->fixed_row_block_size) + "}";
