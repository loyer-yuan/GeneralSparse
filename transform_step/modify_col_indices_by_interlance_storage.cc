...
modify_col_indices_by_interlance_storage::modify_col_indices_by_interlance_storage(shared_ptr<meta_data_set> meta_data_set_ptr, POS_TYPE parent_pos, int target_matrix_id)
    : basic_data_transform_step("modify_col_indices_by_interlance_storage", meta_data_set_ptr)
{
...
void modify_col_indices_by_interlance_storage::run(bool check)
{
    if (check)
    {
    ...
    // 新索引
    vector<unsigned long> new_col_indices(col_indices_ptr->get_len());
    ...
    if (this->parent_pos == GLOBAL_META)
    {
        ...
        {
            //检查是否是BMT_size的倍数
            assert(col_indices_ptr->get_len() % BMT_size->read_integer_from_arr(0) == 0);
        }

        //交错存储间隔
        unsigned long size_of_BMT = BMT_size->read_integer_from_arr(0);
        unsigned long BMT_num = col_indices_ptr->get_len() / size_of_BMT;
        ...
        for (int BMT_id = 0; BMT_id < BMT_num; BMT_id++)
        {
            ...
            {
                new_col_indices[BMT_id + nz_in_BMT * spacing] = col_indices_ptr->read_integer_from_arr(nz_in_BMT + BMT_id * size_of_BMT);
            }
        }
    }
    else if (this->parent_pos == TBLOCK_META || this->parent_pos == WARP_META)
    {
        //输入
        shared_ptr<data_item_record> col_indices_record(new data_item_record(GLOBAL_META, "nz_col_indices", this->target_matrix_id));
        this->source_data_item_ptr_vec.push_back(col_indices_record);
        ...
        for (int parent_blk_id = 0; parent_blk_id < first_BMT_indices_of_parent_ptr->get_len() - 1; parent_blk_id++)
        {
            ...
            for (int BMT_id = 0; BMT_id < BMT_num; BMT_id++)
            {
                for (int nz_in_BMT = 0; nz_in_BMT < size_of_BMT; nz_in_BMT++)
                {
                    new_col_indices[BMT_id + nz_in_BMT * spacing + temp_size] = col_indices_ptr->read_integer_from_arr(nz_in_BMT + BMT_id * size_of_BMT + temp_size);
                }
            }
            if (check)
            {
    ...
    //输出
    shared_ptr<data_item_record> new_nz_col_indices_record(new data_item_record(GLOBAL_META, "nz_col_indices_after_interlance_storage", this->target_matrix_id));
    this->dest_data_item_ptr_vec.push_back(new_nz_col_indices_record);

    // 将new_col_indices转化为通用数组
    shared_ptr<universal_array> new_col_index_ptr(new universal_array(&(new_col_indices[0]), new_col_indices.size(), UNSIGNED_LONG));
    shared_ptr<meta_data_item> item_ptr(new meta_data_item(new_col_index_ptr, GLOBAL_META, "nz_col_indices_after_interlance_storage", this->target_matrix_id));
...
vector<shared_ptr<data_item_record>> modify_col_indices_by_interlance_storage::get_source_data_item_ptr_in_data_transform_step()
{
    assert(this->target_matrix_id >= 0);
    assert(this->parent_pos == GLOBAL_META || this->parent_pos == TBLOCK_META || this->parent_pos == WARP_META);
...
vector<shared_ptr<data_item_record>> modify_col_indices_by_interlance_storage::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    assert(this->target_matrix_id >= 0);
    assert(this->parent_pos == GLOBAL_META || this->parent_pos == TBLOCK_META || this->parent_pos == WARP_META);
    assert(this->is_run == true);
...
string modify_col_indices_by_interlance_storage::convert_to_string()
{
    assert(this->target_matrix_id >= 0);
    assert(this->parent_pos == GLOBAL_META || this->parent_pos == TBLOCK_META || this->parent_pos == WARP_META);

    string return_str = "modify_col_indices_by_interlance_storage::{name:\"" + this->name + "\",target_matrix_id:" + to_string(this->target_matrix_id) + ",parent_pos:" + convert_pos_type_to_string(this->parent_pos) + "}";
