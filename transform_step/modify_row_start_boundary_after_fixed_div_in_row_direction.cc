...
modify_row_start_boundary_after_fixed_div_in_row_direction::modify_row_start_boundary_after_fixed_div_in_row_direction(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, int fixed_row_gap_size)
    : basic_data_transform_step("modify_row_start_boundary_after_fixed_div_in_row_direction", meta_data_set_ptr)
{
...
void modify_row_start_boundary_after_fixed_div_in_row_direction::run(bool check)
{
    if (check)
    {
    ...
    for (unsigned long i = 0; i < empty_bin_flag.size(); i++)
    {
        ...
        {
            // 获取子矩阵行边界的最大子id
            int max_existing_sub_matrix_id_of_start_row_index = this->meta_data_set_ptr->get_max_sub_matrix_id_of_data_item(GLOBAL_META, "begin_row_index");
            ...
            // assert(max_existing_sub_matrix_id_of_start_row_index == max_existing_sub_matrix_id_of_end_row_index);

            // 新的子矩阵的首行索引
            unsigned long new_start_row_index = begin_row_index + i * this->fixed_row_gap_size;
            // unsigned long new_end_row_index = begin_row_index + max_existing_sub_matrix_id_of_start_row_index * (this->fixed_row_gap_size + 1) - 1;

            if (check)
            {
                assert(new_start_row_index <= end_row_index);

                // 加入之前肯定不存在
                assert(this->meta_data_set_ptr->is_exist(GLOBAL_META, "begin_row_index", max_existing_sub_matrix_id_of_start_row_index + 1) == false);
            }

            // 超过被切割的行边界，就替换为别切割的行边界
            // if (new_end_row_index > end_row_index)
            // {
            //     new_end_row_index = end_row_index;
            // }

            // assert(new_start_row_index <= new_end_row_index);

            // 将子矩阵的行边界接到metadata set中
            shared_ptr<meta_data_item> start_row_item(new meta_data_item(((void *)(&new_start_row_index)), UNSIGNED_LONG, GLOBAL_META, "begin_row_index", max_existing_sub_matrix_id_of_start_row_index + 1));

            // 加入
            this->meta_data_set_ptr->add_element(start_row_item);

            // 增加一个输出记录
            shared_ptr<data_item_record> new_start_row_index_record(new data_item_record(GLOBAL_META, "begin_row_index", max_existing_sub_matrix_id_of_start_row_index + 1));
            this->dest_data_item_ptr_vec.push_back(new_start_row_index_record);

            // shared_ptr<meta_data_item> end_row_index(new meta_data_item(((void *)(&new_end_row_index)), UNSIGNED_LONG, GLOBAL_META, "end_row_index", max_existing_sub_matrix_id_of_start_row_index + 1));
            // assert(this->meta_data_set_ptr->is_exist(GLOBAL_META, "end_row_index", max_existing_sub_matrix_id_of_start_row_index + 1));
            // this->meta_data_set_ptr->add_element(end_row_index);

            // shared_ptr<data_item_record> new_end_row_index_record(new data_item_record(GLOBAL_META, "end_row_index", max_existing_sub_matrix_id_of_start_row_index + 1));
...
vector<shared_ptr<data_item_record>> modify_row_start_boundary_after_fixed_div_in_row_direction::get_source_data_item_ptr_in_data_transform_step()
{
    assert(this->target_matrix_id >= 0);
...
vector<shared_ptr<data_item_record>> modify_row_start_boundary_after_fixed_div_in_row_direction::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    assert(this->target_matrix_id >= 0);
    assert(this->is_run == true);
...
// 转化为字符串
string modify_row_start_boundary_after_fixed_div_in_row_direction::convert_to_string()
{
    assert(this->target_matrix_id >= 0);

    // 打印参数
    string return_str = "modify_row_start_boundary_after_fixed_div_in_row_direction::{name:\"" + this->name + "\",target_matrix_id:" + to_string(this->target_matrix_id) + ",fixed_row_gap_size:" + to_string(this->fixed_row_gap_size) + "}";
