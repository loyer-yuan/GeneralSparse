...
segment_empty_row_indices::segment_empty_row_indices(shared_ptr<meta_data_set> meta_data_set_ptr, POS_TYPE pos, int target_matrix_id)
    : basic_data_transform_step("segment_empty_row_indices", meta_data_set_ptr)
{
...
void segment_empty_row_indices::run(bool check)
{
    if (check)
    {
    ...
    vector<unsigned long> segment_empty_row_indices_vec;

    // 遍历所有BMW
    for (unsigned long j = 0; j < first_nzs_ptr->get_len() - 1; j++)
    {
        // 当前BMW的行偏移
        ...
        segment_empty_row_indices_vec.push_back(0);
        for (unsigned int i = cur_nz + 1; i < next_nz; i++)
        {
            ...
            {
                unsigned long relative_result = cur_row - cur_first_row;
                segment_empty_row_indices_vec.push_back(relative_result);
            }
        }
    }
    // 将数据放到metadata set中
    shared_ptr<universal_array> segment_empty_row_indices_ptr(new universal_array(&(segment_empty_row_indices_vec[0]), segment_empty_row_indices_vec.size(), UNSIGNED_LONG));
    shared_ptr<meta_data_item> segment_empty_row_indices_item(new meta_data_item(segment_empty_row_indices_ptr, THREAD_META, "segment_empty_row_indices", this->target_matrix_id));
    this->meta_data_set_ptr->add_element(segment_empty_row_indices_item);

    // 执行记录
    shared_ptr<data_item_record> segment_empty_row_indices_record(new data_item_record(THREAD_META, "segment_empty_row_indices", this->target_matrix_id));
    this->dest_data_item_ptr_vec.push_back(segment_empty_row_indices_record);

    this->is_run = true;
}

vector<shared_ptr<data_item_record>> segment_empty_row_indices::get_source_data_item_ptr_in_data_transform_step()
{
...
vector<shared_ptr<data_item_record>> segment_empty_row_indices::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    assert(this->meta_data_set_ptr != NULL);
    assert(this->meta_data_set_ptr->check());
    assert(this->target_matrix_id >= 0);
...
string segment_empty_row_indices::convert_to_string()
{
    assert(this->target_matrix_id >= 0);

    string return_str = "segment_empty_row_indices::{name:\"" + name + "\",target_matrix_id:" + to_string(target_matrix_id) + ",fixed_row_block_size:" + "}";
