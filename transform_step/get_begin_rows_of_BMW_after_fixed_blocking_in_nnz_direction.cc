...
get_begin_rows_of_BMW_after_fixed_blocking_in_nnz_direction::get_begin_rows_of_BMW_after_fixed_blocking_in_nnz_direction(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, int nnz_per_BMW)
    : basic_data_transform_step("get_begin_rows_of_BMW_after_fixed_blocking_in_nnz_direction", meta_data_set_ptr)
{
...
void get_begin_rows_of_BMW_after_fixed_blocking_in_nnz_direction::run(bool check)
{
    if (check)
    {
        assert(target_matrix_id >= 0);
    ...
    vector<unsigned long> BMW_begin_row_vec;
    ...
    for (unsigned long i = 0; i < nz_row_indices_ptr->get_len(); i += this->nnz_per_BMW)
    {
        ...
        BMW_begin_row_vec.push_back(row_index);
    }
    
    BMW_begin_row_vec.push_back(row_num);
...
vector<shared_ptr<data_item_record>> get_begin_rows_of_BMW_after_fixed_blocking_in_nnz_direction::get_source_data_item_ptr_in_data_transform_step()
{
...
vector<shared_ptr<data_item_record>> get_begin_rows_of_BMW_after_fixed_blocking_in_nnz_direction::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    assert(this->target_matrix_id >= 0);
    assert(this->nnz_per_BMW > 0);
    assert(this->is_run == true);
...
string get_begin_rows_of_BMW_after_fixed_blocking_in_nnz_direction::convert_to_string()
{
    assert(this->target_matrix_id >= 0);
    assert(this->nnz_per_BMW > 0);

    string return_str = "get_begin_rows_of_BMW_after_fixed_blocking_in_nnz_direction::{name:\"" + name + "\",target_matrix_id:" + to_string(target_matrix_id) + ",nnz_per_BMW:" + to_string(this->nnz_per_BMW) + "}";
