...
get_begin_rows_of_BMT_after_fixed_blocking_in_nnz_direction_relative_to_BMW::get_begin_rows_of_BMT_after_fixed_blocking_in_nnz_direction_relative_to_BMW(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, int nnz_per_BMT)
    : basic_data_transform_step("get_begin_rows_of_BMT_after_fixed_blocking_in_nnz_direction_relative_to_BMW", meta_data_set_ptr)
{
...
void get_begin_rows_of_BMT_after_fixed_blocking_in_nnz_direction_relative_to_BMW::run(bool check)
{
    if (check)
    {
        assert(target_matrix_id >= 0);
    ...
    for (unsigned long i = 0; i < nz_row_indices_ptr->get_len(); i += this->nnz_per_BMT)
    {
        ...
        unsigned long current_BMW_first_row = BMW_first_row_indices_ptr->read_integer_from_arr(BMW_id);

        unsigned long row_index = nz_row_indices_ptr->read_integer_from_arr(i);
        BMT_begin_row_vec_relative_to_BMW.push_back(row_index - current_BMW_first_row);
    }
...
vector<shared_ptr<data_item_record>> get_begin_rows_of_BMT_after_fixed_blocking_in_nnz_direction_relative_to_BMW::get_source_data_item_ptr_in_data_transform_step()
{
    // 内容满足要求
    assert(target_matrix_id >= 0);
    assert(nnz_per_BMT > 0);
...
vector<shared_ptr<data_item_record>> get_begin_rows_of_BMT_after_fixed_blocking_in_nnz_direction_relative_to_BMW::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    // 内容满足要求
    assert(target_matrix_id >= 0);
    assert(nnz_per_BMT > 0);
...
string get_begin_rows_of_BMT_after_fixed_blocking_in_nnz_direction_relative_to_BMW::convert_to_string()
{
    assert(target_matrix_id >= 0);
    assert(nnz_per_BMT > 0);

    string return_str = "get_begin_rows_of_BMT_after_fixed_blocking_in_nnz_direction_relative_to_BMW::{name:\"" + name + "\",target_matrix_id:" + to_string(target_matrix_id) + ",nnz_per_BMT:" + to_string(this->nnz_per_BMT) + "}";
