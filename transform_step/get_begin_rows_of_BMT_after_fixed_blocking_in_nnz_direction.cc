...
get_begin_rows_of_BMT_after_fixed_blocking_in_nnz_direction::get_begin_rows_of_BMT_after_fixed_blocking_in_nnz_direction(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, int nnz_per_BMT)
    : basic_data_transform_step("get_begin_rows_of_BMT_after_fixed_blocking_in_nnz_direction", meta_data_set_ptr)
{
...
void get_begin_rows_of_BMT_after_fixed_blocking_in_nnz_direction::run(bool check)
{
    if (check)
    {
        assert(target_matrix_id >= 0);
    ...
    for (unsigned long i = 0; i < nz_row_indices_ptr->get_len(); i += this->nnz_per_BMT)
    {
        ...
        BMT_begin_row_vec.push_back(row_index);
    }
    assert(row_num > BMT_begin_row_vec[BMT_begin_row_vec.size() - 1]);
    BMT_begin_row_vec.push_back(row_num);


    // 将数据放到metadata set中
    shared_ptr<universal_array> BMT_first_row_ptr(new universal_array(&(BMT_begin_row_vec[0]), BMT_begin_row_vec.size(), UNSIGNED_LONG));
    shared_ptr<meta_data_item> BMT_first_row_item(new meta_data_item(BMT_first_row_ptr, THREAD_META, "first_row_indices", this->target_matrix_id));
    this->meta_data_set_ptr->add_element(BMT_first_row_item);

    // 执行记录
    shared_ptr<data_item_record> BMT_first_row_record(new data_item_record(THREAD_META, "first_row_indices", this->target_matrix_id));
    this->dest_data_item_ptr_vec.push_back(BMT_first_row_record);

    this->is_run = true;
}

vector<shared_ptr<data_item_record>> get_begin_rows_of_BMT_after_fixed_blocking_in_nnz_direction::get_source_data_item_ptr_in_data_transform_step()
{
...
vector<shared_ptr<data_item_record>> get_begin_rows_of_BMT_after_fixed_blocking_in_nnz_direction::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    assert(this->target_matrix_id >= 0);
    assert(this->nnz_per_BMT > 0);
    assert(this->is_run == true);
...
string get_begin_rows_of_BMT_after_fixed_blocking_in_nnz_direction::convert_to_string()
{
    assert(this->target_matrix_id >= 0);
    assert(this->nnz_per_BMT > 0);

    string return_str = "get_begin_rows_of_BMT_after_fixed_blocking_in_nnz_direction::{name:\"" + name + "\",target_matrix_id:" + to_string(target_matrix_id) + ",nnz_per_BMT:" + to_string(this->nnz_per_BMT) + "}";
