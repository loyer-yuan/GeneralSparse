...
modify_col_end_boundary_after_div_according_to_row_nz::modify_col_end_boundary_after_div_according_to_row_nz(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, int nz_gap_size, int max_gap, int expansion_rate)
    : basic_data_transform_step("modify_col_end_boundary_after_div_according_to_row_nz", meta_data_set_ptr)
{
...
void modify_col_end_boundary_after_div_according_to_row_nz::run(bool check)
{
    if (check)
    {
    ...
    // 读出列的头部边界
    unsigned long end_col_index = this->meta_data_set_ptr->get_element(GLOBAL_META, "end_col_index", this->target_matrix_id)->get_metadata_arr()->read_integer_from_arr(0);

    // 记录列的头部边界
    shared_ptr<data_item_record> end_col_index_record_ptr(new data_item_record(GLOBAL_META, "end_col_index", this->target_matrix_id));
    this->source_data_item_ptr_vec.push_back(end_col_index_record_ptr);
    ...
    for (unsigned long i = 0; i < div_position.size(); i++)
    {
        ...
        // 写列索引的起始边界
        shared_ptr<meta_data_item> end_col_index_boundary_item(new meta_data_item(((void *)(&end_col_index)), UNSIGNED_LONG, GLOBAL_META, "end_col_index", max_existing_sub_matrix_id_of_end_col_index + 1));
        // 加入之前判断其肯定不存在
        if (check)
        {
            assert(this->meta_data_set_ptr->is_exist(GLOBAL_META, "end_col_index", max_existing_sub_matrix_id_of_end_col_index + 1) == false);
        }
        this->meta_data_set_ptr->add_element(end_col_index_boundary_item);

        // 记录当前输入
        shared_ptr<data_item_record> end_col_index_boundary_record(new data_item_record(GLOBAL_META, "end_col_index", max_existing_sub_matrix_id_of_end_col_index + 1));
        this->dest_data_item_ptr_vec.push_back(end_col_index_boundary_record);
    }
...
vector<shared_ptr<data_item_record>> modify_col_end_boundary_after_div_according_to_row_nz::get_source_data_item_ptr_in_data_transform_step()
{
    assert(this->target_matrix_id >= 0);
...
vector<shared_ptr<data_item_record>> modify_col_end_boundary_after_div_according_to_row_nz::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    assert(this->target_matrix_id >= 0);
    assert(this->is_run == true);
...
string modify_col_end_boundary_after_div_according_to_row_nz::convert_to_string()
{
    assert(this->target_matrix_id >= 0);

    // 打印参数
    string return_str = "modify_col_end_boundary_after_div_according_to_row_nz::{name:\"" + this->name + "\",target_matrix_id:" + to_string(this->target_matrix_id) + ",nz_gap_size:" + to_string(this->nz_gap_size) + "}";
