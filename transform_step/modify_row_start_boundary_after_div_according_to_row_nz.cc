...
modify_row_start_boundary_after_div_according_to_row_nz::modify_row_start_boundary_after_div_according_to_row_nz(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, int nz_gap_size, int max_gap, int expansion_rate)
    : basic_data_transform_step("modify_row_start_boundary_after_div_according_to_row_nz", meta_data_set_ptr)
{
...
void modify_row_start_boundary_after_div_according_to_row_nz::run(bool check)
{
    if (check)
    {
    ...
    for (unsigned long i = 0; i < div_position.size(); i++)
    {
        ...
        // 新的子矩阵的首行索引
        unsigned long new_start_row_index = div_position[i];
...
vector<shared_ptr<data_item_record>> modify_row_start_boundary_after_div_according_to_row_nz::get_source_data_item_ptr_in_data_transform_step()
{
    assert(this->target_matrix_id >= 0);
...
vector<shared_ptr<data_item_record>> modify_row_start_boundary_after_div_according_to_row_nz::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    assert(this->target_matrix_id >= 0);
    assert(this->is_run == true);
...
// 转化为字符串
string modify_row_start_boundary_after_div_according_to_row_nz::convert_to_string()
{
    assert(this->target_matrix_id >= 0);

    // 打印参数
    string return_str = "modify_row_start_boundary_after_div_according_to_row_nz::{name:\"" + this->name + "\",target_matrix_id:" + to_string(this->target_matrix_id) + ",nz_gap_size:" + to_string(this->nz_gap_size) + "}";
