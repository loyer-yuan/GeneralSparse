...
get_begin_nzs_of_BMW_after_nnz_blocking_in_row_direction::get_begin_nzs_of_BMW_after_nnz_blocking_in_row_direction(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, int nnz_per_interval)
    : basic_data_transform_step("get_begin_nzs_of_BMW_after_nnz_blocking_in_row_direction", meta_data_set_ptr)
{
...
void get_begin_nzs_of_BMW_after_nnz_blocking_in_row_direction::run(bool check)
{
    if (check)
    {
    ...
    vector<unsigned long> BMW_begin_nz_vec = get_begin_nzs_of_child_after_balance_blocking_in_row_direction(nnz_of_each_row, this->nnz_per_interval);
...
vector<shared_ptr<data_item_record>> get_begin_nzs_of_BMW_after_nnz_blocking_in_row_direction::get_source_data_item_ptr_in_data_transform_step()
{
...
vector<shared_ptr<data_item_record>> get_begin_nzs_of_BMW_after_nnz_blocking_in_row_direction::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    assert(this->meta_data_set_ptr != NULL);
    assert(this->meta_data_set_ptr->check());
    assert(this->target_matrix_id >= 0);
...
string get_begin_nzs_of_BMW_after_nnz_blocking_in_row_direction::convert_to_string()
{
    assert(this->target_matrix_id >= 0);
    assert(this->nnz_per_interval >= 1);

    string return_str = "get_begin_nzs_of_BMW_after_nnz_blocking_in_row_direction::{name:\"" + name + "\",target_matrix_id:" + to_string(target_matrix_id) + ",nnz_per_interval:" + to_string(this->nnz_per_interval) + "}";
