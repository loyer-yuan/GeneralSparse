...
get_begin_rows_relative_to_parent_after_merge_thread::get_begin_rows_relative_to_parent_after_merge_thread(shared_ptr<meta_data_set> meta_data_set_ptr, POS_TYPE pos, int merge_num, int target_matrix_id)
    : basic_data_transform_step("get_begin_rows_relative_to_parent_after_merge_thread", meta_data_set_ptr)
{
...
void get_begin_rows_relative_to_parent_after_merge_thread::run(bool check)
{
    if (check)
    {
    ...
    {
        unsigned long cur_parent_row = BMT_first_row_indices_ptr->read_integer_from_arr(j);
        for (unsigned long i = j; i < j + this->merge_num && i < BMT_first_row_indices_ptr->get_len() - 1; i++)
        {
            unsigned long cur_row = BMT_first_row_indices_ptr->read_integer_from_arr(i) - cur_parent_row;
    ...
    {
        name = "first_row_indices_relative_to_BMW";
    }
    else if (this->pos == TBLOCK_META)
    {
        name = "first_row_indices_relative_to_BMTB";
    }
    ...
    shared_ptr<meta_data_item> parent_first_row_item(new meta_data_item(parent_first_row_ptr, this->pos, name, this->target_matrix_id));
    this->meta_data_set_ptr->add_element(parent_first_row_item);

    // 执行记录
    shared_ptr<data_item_record> parent_first_row_record(new data_item_record(this->pos, name, this->target_matrix_id));
    this->dest_data_item_ptr_vec.push_back(parent_first_row_record);

    this->is_run = true;
}

vector<shared_ptr<data_item_record>> get_begin_rows_relative_to_parent_after_merge_thread::get_source_data_item_ptr_in_data_transform_step()
{
...
vector<shared_ptr<data_item_record>> get_begin_rows_relative_to_parent_after_merge_thread::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    assert(this->meta_data_set_ptr != NULL);
    assert(this->meta_data_set_ptr->check());
    assert(this->target_matrix_id >= 0);
...
string get_begin_rows_relative_to_parent_after_merge_thread::convert_to_string()
{
    assert(this->target_matrix_id >= 0);

    string return_str = "get_begin_rows_relative_to_parent_after_merge_thread::{name:\"" + name + "\",target_matrix_id:" + to_string(target_matrix_id) + "}";
