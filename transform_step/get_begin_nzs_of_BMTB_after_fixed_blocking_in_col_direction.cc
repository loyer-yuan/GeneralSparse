...
get_begin_nzs_of_BMTB_after_fixed_blocking_in_col_direction::get_begin_nzs_of_BMTB_after_fixed_blocking_in_col_direction(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, int col_size)
    : basic_data_transform_step("get_begin_nzs_of_BMTB_after_fixed_blocking_in_col_direction", meta_data_set_ptr)
{
...
void get_begin_nzs_of_BMTB_after_fixed_blocking_in_col_direction::run(bool check)
{
    if (check)
    {
        assert(target_matrix_id >= 0);
    ...
    // 用一个数组存储每一个BMTB的非零元偏移量
    vector<unsigned long> BMTB_begin_nz_vec;
    BMTB_begin_nz_vec.push_back(0);

    // 遍历每一行，查看对应的BMTB的非零元偏移量，BMTB原则上不存在空的
    for (unsigned long i = 0; i < nnz_of_each_row.size(); i++)
    {
        ...
        {
            // 看看能不能插入完整大小的BMTB
            if (remain_row_size < this->fixed_col_block_size)
            {
                BMTB_begin_nz_vec.push_back(BMTB_begin_nz_vec[BMTB_begin_nz_vec.size() - 1] + remain_row_size);
                remain_row_size = remain_row_size - remain_row_size;
            }
            else
            {
                BMTB_begin_nz_vec.push_back(BMTB_begin_nz_vec[BMTB_begin_nz_vec.size() - 1] + this->fixed_col_block_size);
                remain_row_size = remain_row_size - this->fixed_col_block_size;
            }
        }
        if (check)
        {
    ...
    // 到这里，BMTB_begin_nz_vec的最后一位是总非零元数量
    if (check)
    {
        assert(nz_row_indices_ptr->get_len() == BMTB_begin_nz_vec[BMTB_begin_nz_vec.size() - 1]);
    }

    // 将新的内容放到metadata set中
    shared_ptr<universal_array> BMTB_begin_nz_ptr(new universal_array(&(BMTB_begin_nz_vec[0]), BMTB_begin_nz_vec.size(), UNSIGNED_LONG));
    shared_ptr<meta_data_item> BMTB_begin_nz_item(new meta_data_item(BMTB_begin_nz_ptr, TBLOCK_META, "first_nz_indices", this->target_matrix_id));
    this->meta_data_set_ptr->add_element(BMTB_begin_nz_item);

    // 加入新的record
    shared_ptr<data_item_record> BMTB_begin_nz_record(new data_item_record(TBLOCK_META, "first_nz_indices", this->target_matrix_id));
    this->dest_data_item_ptr_vec.push_back(BMTB_begin_nz_record);

    this->is_run = true;
}

vector<shared_ptr<data_item_record>> get_begin_nzs_of_BMTB_after_fixed_blocking_in_col_direction::get_source_data_item_ptr_in_data_transform_step()
{
...
vector<shared_ptr<data_item_record>> get_begin_nzs_of_BMTB_after_fixed_blocking_in_col_direction::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    assert(this->target_matrix_id >= 0);
    assert(this->fixed_col_block_size > 0);
    assert(this->is_run == true);
...
string get_begin_nzs_of_BMTB_after_fixed_blocking_in_col_direction::convert_to_string()
{
    assert(this->target_matrix_id >= 0);
    assert(this->fixed_col_block_size > 0);

    string return_str = "get_begin_nzs_of_BMTB_after_fixed_blocking_in_col_direction::{name:\"" + name + "\",target_matrix_id:" + to_string(target_matrix_id) + ",col_size:" + to_string(this->fixed_col_block_size) + "}";
