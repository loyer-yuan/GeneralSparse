...
get_begin_rows_of_BMT_after_fixed_blocking_in_col_direction_relative_to_BMTB::get_begin_rows_of_BMT_after_fixed_blocking_in_col_direction_relative_to_BMTB(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, int col_size)
    : basic_data_transform_step("get_begin_rows_of_BMT_after_fixed_blocking_in_col_direction_relative_to_BMTB", meta_data_set_ptr)
{
...
void get_begin_rows_of_BMT_after_fixed_blocking_in_col_direction_relative_to_BMTB::run(bool check)
{
    if (check)
    {
        assert(target_matrix_id >= 0);
    ...
    if (check)
    {
        ...
        if (BMTB_blocking_existing == true)
        {
            ...
            //     if (repeat_num == 2)
            //     {
            //         cout << "get_begin_rows_of_BMT_after_fixed_blocking_in_col_direction_relative_to_BMTB::run(): two BMTBs have mapped to one row" << endl;
            //         assert(false);
            //     }
        ...
        if (BMW_blocking_existing == true)
        {
            ...
            //     if (repeat_num == 2)
            //     {
            //         cout << "get_begin_rows_of_BMT_after_fixed_blocking_in_col_direction_relative_to_BMTB::run(): two BMWs have mapped to one row" << endl;
            //         assert(false);
            //     }
    ...
    vector<unsigned long> BMT_begin_row_vec_relative_to_BMTB;
    ...
    for (unsigned long i = 0; i < BMTB_first_row_indices_ptr->get_len() - 1; i++)
    {
        ...
        for (unsigned long j = BMTB_first_row_index; j < next_BMTB_first_row_index; j++)
        {
            ...
            {
                // 相对索引
                BMT_begin_row_vec_relative_to_BMTB.push_back(j - BMTB_first_row_index);
            }
        }
    }
    if (check)
    {
        assert(BMT_begin_row_vec_relative_to_BMTB.size() > 0);
    }

    // 将相对索引放到metadata set中
    shared_ptr<universal_array> BMT_first_row_index_relative_to_BMTB_ptr(new universal_array(&(BMT_begin_row_vec_relative_to_BMTB[0]), BMT_begin_row_vec_relative_to_BMTB.size(), UNSIGNED_LONG));
    // 行偏移的相对索引
    shared_ptr<meta_data_item> BMT_first_row_index_relative_to_BMTB_item(new meta_data_item(BMT_first_row_index_relative_to_BMTB_ptr, THREAD_META, "first_row_indices_relative_to_BMTB", this->target_matrix_id));
    // 将新的内容写到metadata set中
    this->meta_data_set_ptr->add_element(BMT_first_row_index_relative_to_BMTB_item);

    // 执行记录
    shared_ptr<data_item_record> BMT_first_row_index_relative_to_BMTB_record(new data_item_record(THREAD_META, "first_row_indices_relative_to_BMTB", this->target_matrix_id));
    this->dest_data_item_ptr_vec.push_back(BMT_first_row_index_relative_to_BMTB_record);

    this->is_run = true;
}

vector<shared_ptr<data_item_record>> get_begin_rows_of_BMT_after_fixed_blocking_in_col_direction_relative_to_BMTB::get_source_data_item_ptr_in_data_transform_step()
{
    // 内容满足要求
    assert(target_matrix_id >= 0);
    assert(col_size > 0);
...
vector<shared_ptr<data_item_record>> get_begin_rows_of_BMT_after_fixed_blocking_in_col_direction_relative_to_BMTB::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    // 内容满足要求
    assert(target_matrix_id >= 0);
    assert(col_size > 0);
...
string get_begin_rows_of_BMT_after_fixed_blocking_in_col_direction_relative_to_BMTB::convert_to_string()
{
    assert(target_matrix_id >= 0);
    assert(col_size > 0);

    string return_str = "get_begin_rows_of_BMT_after_fixed_blocking_in_col_direction_relative_to_BMTB::{name:\"" + name + "\",target_matrix_id:" + to_string(target_matrix_id) + ",col_size:" + to_string(this->col_size) + "}";
