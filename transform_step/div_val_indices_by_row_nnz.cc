...
// 构造函数
div_val_indices_by_row_nnz::div_val_indices_by_row_nnz(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, int nz_gap_size, int max_gap, int expansion_rate)
    : basic_data_transform_step("div_val_indices_by_row_nnz", meta_data_set_ptr)
{
...
void div_val_indices_by_row_nnz::run(bool check)
{
    if (check)
    {
    ...
    // 使用一个二维数组来存储分块之后的列索引
    vector<vector<double>> nz_vals_in_each_sub_matrix(div_position.size());
    unsigned long cur_sub_matrix_id = 0;
    // 遍历所有非零元的列索引，以及其对应的行索引
    for (unsigned long i = 0; i < nz_row_indices_ptr->get_len(); i++)
    {
        // 一个非零元的列索引和行索引
        double cur_vals = nz_vals_ptr->read_float_from_arr(i);
        ...
        {
            nz_vals_in_each_sub_matrix[cur_sub_matrix_id].push_back(cur_vals);
        }
    ...
    for (unsigned long i = 0; i < nz_vals_in_each_sub_matrix.size(); i++)
    {
        // 如果当前的行条带是存在的，就要创造新的列索引
        if (nz_vals_in_each_sub_matrix[i].size() != 0)
        {
            ...
            // 当前的子矩阵的列索引
            vector<double> vals_of_this_sub_matrix = nz_vals_in_each_sub_matrix[i];
            ...
            if (nz_vals_ptr->get_data_type() == FLOAT)
            {
...
vector<shared_ptr<data_item_record>> div_val_indices_by_row_nnz::get_source_data_item_ptr_in_data_transform_step()
{
    assert(this->target_matrix_id >= 0);
...
vector<shared_ptr<data_item_record>> div_val_indices_by_row_nnz::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    assert(this->target_matrix_id >= 0);
    assert(this->is_run == true);
...
// 转化为字符串
string div_val_indices_by_row_nnz::convert_to_string()
{
    assert(this->target_matrix_id >= 0);

    // 打印参数
    string return_str = "div_val_indices_by_row_nnz::{name:\"" + this->name + "\",target_matrix_id:" + to_string(this->target_matrix_id) + ",nz_gap_size:" + to_string(this->nz_gap_size) + "}";
