...
get_begin_rows_of_BMT_after_fixed_blocking_in_row_direction_relative_to_BMW::get_begin_rows_of_BMT_after_fixed_blocking_in_row_direction_relative_to_BMW(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, int fixed_row_block_size)
    : basic_data_transform_step("get_begin_rows_of_BMT_after_fixed_blocking_in_row_direction_relative_to_BMW", meta_data_set_ptr)
{
...
void get_begin_rows_of_BMT_after_fixed_blocking_in_row_direction_relative_to_BMW::run(bool check)
{
    if (check)
    {
    ...
    for (unsigned long j = 0; j < BMW_first_row_indices_ptr->get_len() - 1; j++)
    {
        ...
        // 遍历所有的行，一个个分块，起始行为当前遍历warp中非零行号
        for (unsigned long i = BMW_first_row_index; i < next_BMW_first_row_index; i++)
        {
            ...
            if (count == this->fixed_row_block_size)
            {
                ...
                // }
                BMT_begin_row_vec.push_back(i - BMW_first_row_index);
                count = 0;
            }
        }
    }
    ...
    shared_ptr<meta_data_item> BMT_first_row_relative_item(new meta_data_item(BMT_first_row_relative_ptr, THREAD_META, "first_row_indices_relative_to_BMW", this->target_matrix_id));
    this->meta_data_set_ptr->add_element(BMT_first_row_relative_item);

    // 执行记录
    shared_ptr<data_item_record> BMT_first_row_relative_record(new data_item_record(THREAD_META, "first_row_indices_relative_to_BMW", this->target_matrix_id));
    this->dest_data_item_ptr_vec.push_back(BMT_first_row_relative_record);

    this->is_run = true;
}

vector<shared_ptr<data_item_record>> get_begin_rows_of_BMT_after_fixed_blocking_in_row_direction_relative_to_BMW::get_source_data_item_ptr_in_data_transform_step()
{
...
vector<shared_ptr<data_item_record>> get_begin_rows_of_BMT_after_fixed_blocking_in_row_direction_relative_to_BMW::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    assert(this->meta_data_set_ptr != NULL);
    assert(this->meta_data_set_ptr->check());
    assert(this->target_matrix_id >= 0);
...
string get_begin_rows_of_BMT_after_fixed_blocking_in_row_direction_relative_to_BMW::convert_to_string()
{
    assert(this->target_matrix_id >= 0);
    assert(this->fixed_row_block_size >= 1);

    string return_str = "get_begin_rows_of_BMT_after_fixed_blocking_in_row_direction_relative_to_BMW::{name:\"" + name + "\",target_matrix_id:" + to_string(target_matrix_id) + ",fixed_row_block_size:" + to_string(this->fixed_row_block_size) + "}";
