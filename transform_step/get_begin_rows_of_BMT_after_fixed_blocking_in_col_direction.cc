...
get_begin_rows_of_BMT_after_fixed_blocking_in_col_direction::get_begin_rows_of_BMT_after_fixed_blocking_in_col_direction(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, int col_size)
    : basic_data_transform_step("get_begin_rows_of_BMT_after_fixed_blocking_in_col_direction", meta_data_set_ptr)
{
...
void get_begin_rows_of_BMT_after_fixed_blocking_in_col_direction::run(bool check)
{
    if (check)
    {
        assert(target_matrix_id >= 0);
    ...
    if (check)
    {
        ...
        if (BMTB_blocking_existing == true)
        {
            ...
            //     if (repeat_num == 2)
            //     {
            //         cout << "get_begin_rows_of_BMT_after_fixed_blocking_in_col_direction::run(): two BMTBs have mapped to one row" << endl;
            //         assert(false);
            //     }
        ...
        if (BMW_blocking_existing == true)
        {
            ...
            //     if (repeat_num == 2)
            //     {
            //         cout << "get_begin_rows_of_BMT_after_fixed_blocking_in_col_direction::run(): two BMWs have mapped to one row" << endl;
            //         assert(false);
            //     }
    ...
    // BMT没有父块，直接在子矩阵中切块，考虑到空行的存在，行索引并不是从0开始。只有行切块的行偏移才强制从0开始，并且不考虑空行
    ...
    shared_ptr<meta_data_item> BMT_first_row_without_end_item(new meta_data_item(BMT_first_row_without_end_ptr, THREAD_META, "first_row_indices_without_ending", this->target_matrix_id));
    this->meta_data_set_ptr->add_element(BMT_first_row_without_end_item);

    // 执行记录
    shared_ptr<data_item_record> BMT_first_row_without_end_record(new data_item_record(THREAD_META, "first_row_indices_without_ending", this->target_matrix_id));
    this->dest_data_item_ptr_vec.push_back(BMT_first_row_without_end_record);

    this->is_run = true;
}

vector<shared_ptr<data_item_record>> get_begin_rows_of_BMT_after_fixed_blocking_in_col_direction::get_source_data_item_ptr_in_data_transform_step()
{
...
vector<shared_ptr<data_item_record>> get_begin_rows_of_BMT_after_fixed_blocking_in_col_direction::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    assert(this->target_matrix_id >= 0);
    assert(this->col_size > 0);
    assert(this->is_run == true);
...
string get_begin_rows_of_BMT_after_fixed_blocking_in_col_direction::convert_to_string()
{
    assert(this->target_matrix_id >= 0);
    assert(this->col_size > 0);

    string return_str = "get_begin_rows_of_BMT_after_fixed_blocking_in_col_direction::{name:\"" + name + "\",target_matrix_id:" + to_string(target_matrix_id) + ",col_size:" + to_string(this->col_size) + "}";
