...
get_begin_nzs_of_BMT_after_nnz_blocking_in_row_direction_in_BMTB::get_begin_nzs_of_BMT_after_nnz_blocking_in_row_direction_in_BMTB(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, int nnz_per_interval)
    : basic_data_transform_step("get_begin_nzs_of_BMT_after_nnz_blocking_in_row_direction_in_BMTB", meta_data_set_ptr)
{
...
void get_begin_nzs_of_BMT_after_nnz_blocking_in_row_direction_in_BMTB::run(bool check)
{
    if (check)
    {
    ...
    vector<unsigned long> BMT_begin_nz_vec = get_begin_nzs_of_child_after_balance_blocking_in_row_direction_in_parent(BMTB_first_row_indices_ptr, BMTB_first_nzs_ptr, nnz_of_each_row, this->nnz_per_interval);
...
vector<shared_ptr<data_item_record>> get_begin_nzs_of_BMT_after_nnz_blocking_in_row_direction_in_BMTB::get_source_data_item_ptr_in_data_transform_step()
{
...
vector<shared_ptr<data_item_record>> get_begin_nzs_of_BMT_after_nnz_blocking_in_row_direction_in_BMTB::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    assert(this->meta_data_set_ptr != NULL);
    assert(this->meta_data_set_ptr->check());
    assert(this->target_matrix_id >= 0);
...
string get_begin_nzs_of_BMT_after_nnz_blocking_in_row_direction_in_BMTB::convert_to_string()
{
    assert(this->target_matrix_id >= 0);
    assert(this->nnz_per_interval >= 1);

    string return_str = "get_begin_nzs_of_BMT_after_nnz_blocking_in_row_direction_in_BMTB::{name:\"" + name + "\",target_matrix_id:" + to_string(target_matrix_id) + ",nnz_per_interval:" + to_string(this->nnz_per_interval) + "}";
