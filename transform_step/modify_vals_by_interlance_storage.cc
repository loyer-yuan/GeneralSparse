...
modify_vals_by_interlance_storage::modify_vals_by_interlance_storage(shared_ptr<meta_data_set> meta_data_set_ptr, POS_TYPE parent_pos, int target_matrix_id)
    : basic_data_transform_step("modify_vals_by_interlance_storage", meta_data_set_ptr)
{
...
void modify_vals_by_interlance_storage::run(bool check)
{
    if (check)
    {
    ...
    // 新索引
    vector<double> new_vals(vals_ptr->get_len());
    ...
    if (parent_pos == GLOBAL_META)
    {
        ...
        if (check)
        {
            assert(vals_ptr->get_len() % BMT_size->read_integer_from_arr(0) == 0);
        }

        //交错存储间隔
        unsigned long size_of_BMT = BMT_size->read_integer_from_arr(0);
        unsigned long BMT_num = vals_ptr->get_len() / size_of_BMT;
        ...
        for (int BMT_id = 0; BMT_id < BMT_num; BMT_id++)
        {
            ...
            {
                new_vals[BMT_id + nz_in_BMT * spacing] = vals_ptr->read_float_from_arr(nz_in_BMT + BMT_id * size_of_BMT);
            }
        }
    }
    else if (parent_pos == TBLOCK_META || parent_pos == WARP_META)
    {
        ...
        for (int parent_blk_id = 0; parent_blk_id < first_BMT_indices_of_parent_ptr->get_len() - 1; parent_blk_id++)
        {
            ...
            for (int BMT_id = 0; BMT_id < BMT_num; BMT_id++)
            {
                for (int nz_in_BMT = 0; nz_in_BMT < size_of_BMT; nz_in_BMT++)
                {
                    new_vals[BMT_id + nz_in_BMT * spacing + temp_size] = vals_ptr->read_float_from_arr(nz_in_BMT + BMT_id * size_of_BMT + temp_size);
                }
            }
            if (check)
            {
    ...
    //输出
    shared_ptr<data_item_record> new_nz_vals_record(new data_item_record(GLOBAL_META, "nz_vals_after_interlance_storage", this->target_matrix_id));
    this->dest_data_item_ptr_vec.push_back(new_nz_vals_record);

    // 将new_vals转化为通用数组
    shared_ptr<universal_array> new_vals_ptr(new universal_array(&(new_vals[0]), new_vals.size(), DOUBLE));
    ...
    shared_ptr<meta_data_item> item_ptr(new meta_data_item(new_vals_ptr, GLOBAL_META, "nz_vals_after_interlance_storage", this->target_matrix_id));
...
vector<shared_ptr<data_item_record>> modify_vals_by_interlance_storage::get_source_data_item_ptr_in_data_transform_step()
{
    assert(this->target_matrix_id >= 0);
...
vector<shared_ptr<data_item_record>> modify_vals_by_interlance_storage::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    assert(this->target_matrix_id >= 0);
    assert(this->parent_pos == GLOBAL_META || parent_pos == TBLOCK_META || parent_pos == WARP_META);
    assert(this->is_run == true);
...
string modify_vals_by_interlance_storage::convert_to_string()
{
    assert(this->target_matrix_id >= 0);
    assert(this->parent_pos == GLOBAL_META || parent_pos == TBLOCK_META || parent_pos == WARP_META);

    string return_str = "modify_vals_by_interlance_storage::{name:\"" + this->name + "\",target_matrix_id:" + to_string(this->target_matrix_id) + ",parent_pos:" + convert_pos_type_to_string(this->parent_pos) + "}";
