...
get_begin_nzs_of_BMT_after_fixed_blocking_in_row_direction_in_BMTB::get_begin_nzs_of_BMT_after_fixed_blocking_in_row_direction_in_BMTB(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, int fixed_row_block_size)
    : basic_data_transform_step("get_begin_nzs_of_BMT_after_fixed_blocking_in_row_direction_in_BMTB", meta_data_set_ptr)
{
...
void get_begin_nzs_of_BMT_after_fixed_blocking_in_row_direction_in_BMTB::run(bool check)
{
    if (check)
    {
    ...
    for (unsigned long j = 0; j < BMTB_first_row_indices_ptr->get_len() - 1; j++)
    {
        ...
        BMT_begin_nz_vec.push_back(BMTB_first_nz);
        // 遍历所有的行
        for (unsigned long i = BMTB_first_row_index; i < next_BMTB_first_row_index; i++)
        {
            ...
            //}
            //正好与BMTB边界对齐不必计算，下一个BMTB会插入首个非零元
            if (row_count == this->fixed_row_block_size && i != (next_BMTB_first_row_index - 1))
            {
                BMT_begin_nz_vec.push_back(BMTB_first_nz + nz_count);
                row_count = 0;
            }
        }
    }

    BMT_begin_nz_vec.push_back(BMTB_first_nzs_ptr->read_integer_from_arr(BMTB_first_nzs_ptr->get_len() - 1));
...
vector<shared_ptr<data_item_record>> get_begin_nzs_of_BMT_after_fixed_blocking_in_row_direction_in_BMTB::get_source_data_item_ptr_in_data_transform_step()
{
...
vector<shared_ptr<data_item_record>> get_begin_nzs_of_BMT_after_fixed_blocking_in_row_direction_in_BMTB::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    assert(this->meta_data_set_ptr != NULL);
    assert(this->meta_data_set_ptr->check());
    assert(this->target_matrix_id >= 0);
...
string get_begin_nzs_of_BMT_after_fixed_blocking_in_row_direction_in_BMTB::convert_to_string()
{
    assert(this->target_matrix_id >= 0);
    assert(this->fixed_row_block_size >= 1);

    string return_str = "get_begin_nzs_of_BMT_after_fixed_blocking_in_row_direction_in_BMTB::{name:\"" + name + "\",target_matrix_id:" + to_string(target_matrix_id) + ",fixed_row_block_size:" + to_string(this->fixed_row_block_size) + "}";
