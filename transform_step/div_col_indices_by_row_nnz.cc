...
// 构造函数
div_col_indices_by_row_nnz::div_col_indices_by_row_nnz(shared_ptr<meta_data_set> meta_data_set_ptr, int target_matrix_id, int nz_gap_size, int max_gap, int expansion_rate)
    : basic_data_transform_step("div_col_indices_by_row_nnz", meta_data_set_ptr)
{
...
void div_col_indices_by_row_nnz::run(bool check)
{
    if (check)
    {
        ...
        assert(this->max_gap >= nz_gap_size);
    ...
    // 使用一个二维数组来存储分块之后的列索引
    vector<vector<unsigned long>> nz_col_indices_in_each_sub_matrix(div_position.size());
...
vector<shared_ptr<data_item_record>> div_col_indices_by_row_nnz::get_source_data_item_ptr_in_data_transform_step()
{
    assert(this->target_matrix_id >= 0);
...
vector<shared_ptr<data_item_record>> div_col_indices_by_row_nnz::get_dest_data_item_ptr_in_data_transform_step_without_check()
{
    assert(this->target_matrix_id >= 0);
    assert(this->is_run == true);
...
// 转化为字符串
string div_col_indices_by_row_nnz::convert_to_string()
{
    assert(this->target_matrix_id >= 0);

    // 打印参数
    string return_str = "div_col_indices_by_row_nnz::{name:\"" + this->name + "\",target_matrix_id:" + to_string(this->target_matrix_id) + ",nz_gap_size:" + to_string(this->nz_gap_size) + "}";
