...
bool cmp(const tuple<int, int, float>& a, const tuple<int, int, float>& b)
...
void get_matrix_index_and_val_from_file(string coo_file_name, vector<unsigned long> &row_index_vec, vector<unsigned long> &col_index_vec, vector<float> &float_val_vec, vector<double> &double_val_vec, data_type val_data_type, unsigned long &max_row_index, unsigned long &max_col_index)
{
    ...
    bool tran = (get_config()["PERFORMANCE_FLAG"].as_string() == "graph" && (get_config()["Graph_Algorithm"].as_string() == "BFS" || get_config()["Graph_Algorithm"].as_string() == "PR"))?true:false;
    max_col_index = 0;
    max_row_index = 0;
    cout<<coo_file_name<<endl;
    ...
    vector<tuple<unsigned long,unsigned long, float>> cc_vec;
    if (infile.is_open())
    {
        while (infile.good() && !infile.eof())
        {
            ...
            if (row_index_vec.size() != 0)
            {
                ...
                {
                    assert(col_index >= col_index_vec[col_index_vec.size() - 1]);
                }
            }
            if (graph_flag == true)
            {
                if(row_index != col_index)
                {
                    ...
                    if (cc_flag == true)
                    {
                        tuple<unsigned long,unsigned long,float> item1 = make_tuple(row_index, col_index, 1);
                        tuple<unsigned long,unsigned long,float> item2 = make_tuple(col_index, row_index, 1);
                        cc_vec.push_back(item1);
                        cc_vec.push_back(item2);
                    }
                    else
                    {
                        ...
                        //     {
                        //         double_val_vec.push_back(0.85*val);
                        //     }
                        //     else if (val_data_type == FLOAT)
                        //     {
                        //         float_val_vec.push_back(0.85*val);
                        //     }
                        //     else
                        //     {
    ...
    unsigned long last_col=0;
    bool first_flag=true;

    // CC输出的排序去重（把有向变无向回导致重复）
    if(cc_flag == true && graph_flag == true)
    {
        sort(cc_vec.begin(), cc_vec.end(), cmp);
        for(unsigned long i=0; i<cc_vec.size(); i++)
        {
            if(first_flag == true)
            {
                row_index_vec.push_back(get<0>(cc_vec[i]));
                col_index_vec.push_back(get<1>(cc_vec[i]));
                float_val_vec.push_back(get<2>(cc_vec[i]));
            ...
            {
                if (get<0>(cc_vec[i]) == last_row && get<1>(cc_vec[i]) == last_col)
                {
                }
                else
                {
    ...
    // 检查取出的内容数量
    assert(row_index_vec.size() == col_index_vec.size() && col_index_vec.size());
...
void get_matrix_index_and_val_from_file_int(string coo_file_name, vector<unsigned int> &row_index_vec, vector<unsigned int> &col_index_vec, vector<float> &float_val_vec, vector<double> &double_val_vec, data_type val_data_type, unsigned long &max_row_index, unsigned long &max_col_index)
{
    assert(val_data_type == FLOAT || val_data_type == DOUBLE);
    assert(row_index_vec.size() == 0 && col_index_vec.size() == 0 && float_val_vec.size() == 0 && double_val_vec.size() == 0);
    ...
    if (infile.is_open())
    {
        while (infile.good() && !infile.eof())
        {
            ...
            // 佛罗里达矩阵先行索引，然后是列索引
            unsigned int row_index = atol(sv[0].c_str()) - 1;
            unsigned int col_index = atol(sv[1].c_str()) - 1;
...
unsigned int byte_num_of_data_type(data_type type)
{
    if (type == CHAR)
    {
        return sizeof(char);
...
bool check_data_type(data_type type)
{
    if (type >= 0 && type < 35)
    {
        return true;
...
void print_arr_to_file_with_data_type(void *arr, data_type type, unsigned long length, string file_name)
{
    ...
    if (!arrWrite)
    {
        cout << "print_arr_to_file_with_data_type: open file error" << endl;
        assert(false);
    }

    cout << "print_arr_to_file_with_data_type:file_name:" << file_name << endl;
    ...
    else if (type == DOUBLE || type == FLOAT)
    {
        ...
        {
            // 防止精度导致的bug
            double output_num = read_double_from_array_with_data_type(arr, type, i);

            if (output_num <= 0.0000000001)
            {
                output_num = 0.0;
            }
