...
typedef struct code_builder
{
    ...
    // 更细一层对行的共享决定了上一层是不是需要归约。
    // 原子型归约需要更细一层直接将结果写到下一层的输出中，共享内存型归约需要将更细一层的结果全部保留下来
...
string code_of_template_kernal(void *output_template, template_type type, unsigned long dense_block_id, string sr = get_config()["Semiring"].as_string());
